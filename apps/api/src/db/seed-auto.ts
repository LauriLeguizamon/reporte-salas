...
export function seedIfEmpty() {
  const count = db.prepare('SELECT COUNT(*) as n FROM provincia').get() as { n: number };
  if (count.n > 0) return;
  ...
  const seed = db.transaction(() => {
    ...
    const movies: Record<number, string> = {
      [s1]: 'Michael', [s2]: 'Michael', [s3]: 'Super Mario Galaxy: La pelicula',
      [s4]: 'Thunderbolts*', [s5]: 'Lilo & Stitch', [s6]: 'Sinners',
      [s7]: 'Michael', [s8]: 'Thunderbolts*', [s9]: 'Lilo & Stitch', [s10]: 'Sinners',
    };

    const weekendMultiplier = [1.0, 1.3, 1.8, 1.6, 0.7, 0.6, 0.5];
    const baseEntradas: Record<number, number> = {
      [s1]: 9, [s2]: 5, [s3]: 7, [s4]: 120, [s5]: 95,
      [s6]: 85, [s7]: 200, [s8]: 180, [s9]: 150, [s10]: 130,
    };
    const avgTicketPrice = 9000;
